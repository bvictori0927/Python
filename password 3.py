...
#using boolean fields to simplify handling them later
use_uppercase = False
use_lowercase = False
use_numbers = False
use_specials = False
...
while True: # until we get a character that is either Y or N, continue
	...
	if uppercase.upper() == "Y":
		use_uppercase = True
		break
	#if we got a N just leave since it is already false
...
if use_uppercase: password_chars += string.ascii_letters.lower()
if use_lowercase: password_chars += string.ascii_letters.upper()
